...
const LogRegistros = () => {
  ...
  const [statuses] = useState(["CR", "ER", "MR", "AC", "NA", "NC","AR"]);
  ...
  const delayedRequest = debounce(() => {
    if(logRegistroCompras.length===0){ 
      LogRegistroCompras();}
  }, 500)
  ...
  const exportExcel = () => {
    ...
    import("xlsx").then((xlsx) => {
      ...
      saveAsExcelFile(excelBuffer, "log_registro_proveedores");
    });
  };
  /* guarda el excel */
  ...
  const obtenerResponsable = () => {
    if (responsables.length === 0) {
      const nombresUnicos = logRegistroCompras.reduce(
