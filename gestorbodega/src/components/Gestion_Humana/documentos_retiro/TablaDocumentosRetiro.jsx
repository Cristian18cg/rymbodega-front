...
export const TablaDocumentosRetiro = ({
  documento,
  nombrecompleto,
  auxiliar,
}) => {
  ...
  const aceptar_archivo = async (archivo, documento) => {
    try {
      const user = usuario;
      await aprobarArchivoRetiro(archivo.nombre_archivo, documento, user);
      if (
        respuestaAprobar === "" ||
  ...
  const traerArchivos = async () => {
    ...
    if (archivosRetiro[datos] || datos === undefined) {
    } else {
      try {
        await BuscarArchivosRetiro(datos);
      } catch (error) {
        ...
        <div className="col-md-12 ">
          ...
          {busquedaRealizada ? (
            <DataTable
              ...
              value={archivosRetiro[documento]}
              paginator
              rows={5}
