...
const PedidosPovider = ({ children }) => {
  ...
  const Listar_entregadores_rutas = useCallback(async () => {
    try {
  ...
  const Listar_entregadores = useCallback(async () => {
    try {
      const tokenDeAcceso = token;
  ...
  const obtener_ruta = useCallback(
    async (documento) => {
      try {
        const tokenDeAcceso = token;
        const response = await clienteAxios.get(
  ...
  const historico_entregas = useCallback(
    async (fechaInicio = "", fechaFin = "", fecha = "") => {
      try {
        const tokenDeAcceso = token;
  ...
  const CrearEntregador = useCallback(
    async (datos) => {
      const tokenDeAcceso = token;
      try {
    ...
    [FuncionErrorToken, Listar_entregadores, token, usuario]
  );
  /* Funcion para listar los pedidos por documento */
  const Listar_pedidos = useCallback(
    async (documento) => {
      try {
        const tokenDeAcceso = token;
        const response = await clienteAxios.get(
  ...
  const actualizar_pedidos = useCallback(
    async (id, field, dato, documento, efectivo, numeroRuta) => {
      try {
        const tokenDeAcceso = token;
  ...
  const completar_ruta = useCallback(
    async (ruta, documento) => {
      try {
        const tokenDeAcceso = token;
        const response = await clienteAxios.put(
  ...
  const eliminar_pedido = useCallback(
    async (id, documento) => {
      try {
        const tokenDeAcceso = token;
  ...
  /* Funcion para listar las rutas por entregador del dia */
  const Estadisticas = useCallback(async () => {
    try {
      const tokenDeAcceso = token;
